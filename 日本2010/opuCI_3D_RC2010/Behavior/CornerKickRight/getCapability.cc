...
Behavior::ConfidenceInterval CornerKickRight::getCapability(rf<State> s, rf<Goal> g)
{
  WorldModel& wm = WorldModel::getInstance();
  rf<StateVarNode> node = rf_cast<StateVarNode>(g->findDeep("CornerKick"));
  Types::Object corner;
  if( ! ballKickableByRight() || ! node )
  ...
  else if ( corner == Types::FLAG2THEM )
  {
    ...
    std::cerr << "(CornerKickRight) error... unknown corner" <<std::endl;
