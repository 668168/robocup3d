...
void RoleRightForward::doKick()
{
   ...
   std::cout<<"RoleRightForward doKick execute"<<std::endl;   
};

void RoleRightForward::doMove()
{
...
void RoleRightForward::doStandUp()
{
    std::cout<<"RoleRightForward doStandup execute"<<std::endl;
...
void RoleRightForward::doStop()
{
//     BhvFocusBall().execute();
    std::cout<<"RoleRightForward doStop execute"<<std::endl;
    WorldModel& wm   = WorldModel::instance();
    Walk&       walk = Walk::instance();

    if (walk.IsPrepareDone() && wm.isStable())
    {
        cout <<"RoleRightForward::doStop : 'prepare walk done' "<<endl;
