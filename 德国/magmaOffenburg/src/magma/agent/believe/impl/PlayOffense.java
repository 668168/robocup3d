 ...
 * 
 */
public class PlayOffense extends Believe
{

	public PlayOffense(IAgentModel agent, IGlobalMap worldModel)
	{
		super(IBelieve.PLAY_OFFENSE, agent, worldModel);
	}

	@Override
	public float getTruthValue()
	{
		char side = worldModel.getThisPlayer().getSide();
		double ballX = worldModel.getBall().getPosition().getX();
		// int id = worldModel.getThisPlayer().getID();

		if (ballX > 2 && side == IMagmaConstants.LEFT_SIDE) {
			return 1.0f;
		} else if (ballX < -2 && side == IMagmaConstants.RIGHT_SIDE) {
			return 1.0f;
		}
