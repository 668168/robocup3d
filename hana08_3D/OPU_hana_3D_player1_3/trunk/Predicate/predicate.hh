...
namespace bats {
  ...
  class Predicate;
  /**
   *  \depricated rPredicate is depricated, you should use rf<Predicate>.
   */
  //  typedef rf<Predicate> rPredicate;
  ...
  public:
    ...
    */
    ...
    {
//      std::cout << "In predicate.hh" << std::endl;
//      std::cout << "  Beam to (" << vec.getX() << ", " << vec.getY() << ", " << vec.getZ() << ")" << std::endl;
      pushLeaf(vec.getX());
      pushLeaf(vec.getY());
      pushLeaf(vec.getZ());
    }
  ...
  /**
   *  So the predicate can be written to the stdout.
   */
